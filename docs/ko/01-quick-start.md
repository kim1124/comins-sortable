...
이 가이드는 `main`의 0.1.2 후보를 기준으로 합니다. 현재 npm 배포본은 0.1.1이며,
새로운 0.1.2 정렬 기능은 아래 저장소 Playground에서 확인합니다.
...
모든 Playground 카드는 왼쪽 핸들로 정렬하고 본문에서 텍스트를 선택·복사합니다.
이는 데모의 구성 방식이며, 라이브러리 소비자는 `handle` 옵션으로 적용합니다.
