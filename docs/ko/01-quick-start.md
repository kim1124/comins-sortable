...
Playground는 기본적으로 왼쪽 핸들로 정렬하고 본문에서 텍스트를 선택·복사합니다.
**드래그 시작 영역** 예제는 제목·카드 전체 활성화도 비교합니다. 이는 데모의 구성
방식이며 소비자는 `handle` 옵션으로 적용합니다. 모드별 차이와 현재 Safari 제목
드래그 제한은 [핸들과 수락 정책](./09-handle-acceptance.md)을 참조합니다.
